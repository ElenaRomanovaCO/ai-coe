...
- 2026-06-08: **UI slice DONE** (lint/build/synth/test clean, NOT deployed). Browse `/modules/asset-library` (server fetch → `AssetBrowser` client filters: industry/type/stage/text, instant client-side over seed list) + detail `/modules/asset-library/[id]` (server-rendered `MarkdownRenderer` (react-markdown+remark-gfm+rehype-sanitize, explicit element styling — no typography plugin) + `FrontmatterPanel` + `AssetActions`). `actions.ts` server actions invoke AGENT-03 via `invokeModule` (lambda:Invoke). Dashboard links to the library.
- 2026-06-08: **Save/Rate/Flag = AGENT-03 write ops, NOT SSR-direct-S3** (spec step 9 said server actions write S3 — but the web has no S3 per `web-chat-transport`). Added ops save_asset/rate_asset/flag_asset to AGENT-03: user profile `users/{slug}.json` (sessions bucket), aggregate sidecar `assets/_metadata/{id}.json` (vault). `get_asset` now returns `aggregates` + optional per-user `user` block. Demo-grade read-modify-write (no locking).
- 2026-06-08: **Two more "spec was wrong about IAM/contract" fixes**: (1) `amplify-ssr-role` got `lambda:InvokeFunction` on module-agents (spec said "no role changes" — web read actions invoke it directly); (2) orchestrator `asset_library_url` rewired from `/modules/module-2?file=…` → `/modules/asset-library/{id}` so chat citations resolve. **Contract nailed locally** (was deploy-time-only): `test_contract.py` wires orchestrator `ModuleInvoker` → `router.route` → AGENT-03 through a fake Lambda, proving `{agent_id, args}` + every payload shape (query→search, asset_id|id→get, filters→list, {}→list-all, disabled→stub-without-invoke). invoke_module tool description also steers the LLM toward op/query/asset_id/filter shapes.
