...
- 2026-06-06: **Agent runtime = Bedrock Converse tool-loop, NOT Strands** (cross-cutting). The foundation built `BaseAgent`/`BedrockClient` on Converse; adopting Strands would fork the runtime for no demo-grade benefit. AGENT-01 and all later module/worker agents follow this. Full rationale in `vault/decisions/agent-runtime.md`.
- 2026-06-06: **Proxy Lambda name = `aicoe-fargate-orchestrator-endpoint-lambda`** (NOT the spec's `aicoe-orchestrator-proxy`). The foundation already wired this name into `aicoe-amplify-ssr-role` (IAM invoke grant) and `web/lib/aws.ts` (`orchestratorFunctionName()`); reusing it avoids breaking deployed permissions. `aicoe-fargate-orchestrator-role` already carries `bedrock:ApplyGuardrail` + streaming perms, so that IAM delta is a no-op.
- 2026-06-06: **Web chat = streaming route handler + client-side SSE/history, NOT a server action.** `web/app/api/chat/route.ts` is a pure SSE passthrough (returns `new Response(upstream.body)`, no buffering; gates on `auth_ok` cookie since `/api` is middleware-excluded). `web/lib/sse.ts` parses the stream; `ChatDock.tsx` renders token-by-token + citation badges, mounted in the `(authenticated)` layout. Server Actions can't stream incrementally → route handler instead (supersedes spec's `app/actions/chat.ts`). History kept in `sessionStorage` (per-tab: survives refresh, new tab = new session) because the SSR role has no S3 — orchestrator still persists to S3 for durability. Rationale: `vault/decisions/web-chat-transport.md`. Smoke-test target: tokens stream incrementally in the browser through Amplify SSR.
- 2026-06-06: **POC runtime = streaming Lambda, NOT Fargate+ALB+proxy** (refines AD-01/AD-02 for POC; ~$0 idle). Orchestrator = container Lambda (FastAPI `server.py` under Lambda Web Adapter) + Function URL `InvokeMode=RESPONSE_STREAM`, IAM auth. The endpoint Lambda IS the orchestrator (runs `agent_01_chat.stream()`); ALB + separate proxy dropped (`agents/proxy/` not created). Transport changed from `InvokeWithResponseStream` → Function URL because Python can't stream from the managed runtime and LWA streaming requires a Function URL — SSR grant is now `lambda:InvokeFunctionUrl`, URL injected to Amplify as `AICOE_ORCHESTRATOR_URL`, `web/lib/aws.ts` uses SigV4 streaming fetch. Config hot-reload: per-invocation `refresh_if_stale` on Lambda (frozen env), daemon kept in `server.py` for local/Fargate. Fargate kept as un-deployed scale-up path in `infra/stacks/agents_fargate.py` (same image). Full rationale in `vault/decisions/poc-runtime.md`. Steps 3-4 below are superseded by this.
