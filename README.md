...
```
infra/      AWS CDK (Python) — storage, IAM, events, observability, guardrails
...
| Frontend     | Next.js 16 (App Router), Tailwind, shadcn/ui → Vercel             |
...
> ⚠️ Deploy steps create billable AWS/Vercel resources and are **not run by the
> build** — run them yourself with credentials in hand.
...
4. **Vercel:** create the project, enable AWS OIDC, set `APP_PASSWORD` and the
   `AICOE_VERCEL_ROLE_ARN` / `AICOE_ORCHESTRATOR_FN` env vars. Pass the project's
   OIDC issuer/sub to CDK via `--context vercel_oidc_sub=...` so the role trust
   policy matches.
...
- `cd infra && npx aws-cdk synth` → all 5 stacks synthesize
